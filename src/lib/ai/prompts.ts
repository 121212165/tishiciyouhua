...
const OPTIMIZE_SYSTEM_PROMPT = `# 角色 (Role)
...
const STYLES: Record<string, { name: string; instruction: string }> = {
  concise: {
    name: '简洁',
    instruction: '保持精炼，去除冗余表述，只保留核心指令和必要约束。使用短句和关键词，适合快速执行的场景。',
  },
  detailed: {
    name: '详细',
    instruction: '全面补充上下文、角色设定、格式要求和约束条件。使用 CO-STAR 框架的完整结构，适合需要高质量输出的复杂任务。',
  },
  creative: {
    name: '创意',
    instruction: '添加创意思维角度，鼓励多角度分析和创新表达。引入类比、比喻和跨领域联想，突破常规思路框架。',
  },
  academic: {
    name: '学术',
    instruction: '采用学术论文的严谨风格，强调逻辑论证和结构化表达。要求引用来源、使用精确术语、保持客观中立的分析立场。',
  },
  technical: {
    name: '技术文档',
    instruction: '采用技术文档的精确风格，适合编程、架构设计和技术方案场景。要求代码示例、API 说明、错误处理方案。',
  },
  business: {
    name: '商务',
    instruction: '采用专业商务沟通风格，强调结果导向和可执行性。使用结构化的商业分析框架，语言简洁专业。',
  },
  instruction: {
    name: '指令式',
    instruction: '将提示词转化为分步骤的可执行指令序列。每一步使用动词开头，明确输入输出，适合工作流自动化场景。',
  },
}
...
export function buildSystemPromptWithLanguage(style: Style, inputText: string): string {
  ...
  if (!styleConfig) throw new Error(`Unknown style: ${style}`)
